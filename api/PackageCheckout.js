...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    try {
      ...
      console.log("Destination address inserted with ID:", destination_address_id);
      ...
      if (!taxResult.length) {
        return res.status(400).json({ success: false, error: 'Tax not found for the given state' });
      }
