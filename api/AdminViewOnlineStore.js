  ...
  if (req.method === 'GET') {
    ...
    try {
      ...
      const [rows] = await connection.execute(
        `SELECT p.tracking_number, p.status, ifs.stock, ifs.item_category, c.email, t.transaction_date, ifs.item_name, item_price, ip.item_amount_purchased, ifs.item_id, p.po_id,
...
WHERE p.type IS NULL
  AND p.tracking_number = t.packages_tracking_number;`
      );
